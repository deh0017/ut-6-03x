	...
	for (i = 0; i < SCREENW; i++) {
		...
		if (Missile[i].life) {
			...
			if (Bunker.life && i >= 30 && i <= 51) {
				...
				Missile[i].life = 0;
                // change the bunker's image and erase the missile on the field
				Nokia5110_PrintBMP(i, Missile[i].y, Missile2, 0);
				Nokia5110_PrintBMP(Bunker.x, Bunker.y, Bunker.image[BunkerFrame], 0);
				Nokia5110_DisplayBuffer();
			}
			...
			/* // if the missile is out of the field
			if (Missile[i].y < 7) {
				// set its life back to zero
				Missile[i].life = 0;
			} */
		...
		if (Laser[i].life) {
            ...
            && i >= Ship.x - 1 && i <= Ship.x + 18) {
				...
				Laser[i].life = 0;
                // change the ship's frame and erase the laser on the field
			...
			&& i >= 30 && i <= 51) {
                ...
                // set the laser to be non-existent
				Laser[i].life = 0;
                // change the bunker's frame and erase the laser on the field
				Nokia5110_PrintBMP(i, Laser[i].y, Laser1, 0);
				Nokia5110_PrintBMP(Bunker.x, Bunker.y, Bunker.image[BunkerFrame], 0);
				Nokia5110_DisplayBuffer();
			}
...
/* 			// if the laser is out of the field
			if (Laser[i].y > 49) {
				// set its life back to zero
				Laser[i].life = 0;
			} */
